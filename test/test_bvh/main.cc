...
struct TriangleCollide
{
  ...
  inline void inverse_dir(const Vec3 &dir)
...
inline bool TriangleCollide::operator()(
  const Box3 &box,
  const Vec3 &org,
  const Vec3 &dir,
  double &dist) const
...
inline bool TriangleCollide::operator() (int fid, const Vec3 &org, const Vec3 &dir, double &dist) const
{
  ...
  bool hit = intersecting(
    vs[f[0]], vs[f[1]], vs[f[2]],
    org, dir, dist, true);
  if (hit) fc = fid;
  return hit;
}

static void generate_model(std::vector<Vec3> &vs, std::vector<Int3> &fs)
{
...
int main(int argc, const char **argv)
{
  ...
  double dist { 1e10 };
  const Vec3 org {-2,0,0 };
  const Vec3 dir { 1,0,0 };
  collide.inverse_dir(dir);
  intersect(bvh, collide, org, dir, dist, fids.begin());
